...
function api_password_lost($request)
{
    ...
    if (empty($login)) {
        $response = new WP_Error('error', 'Informe email ou login', ['status' => 406]);
    ...
    $body = $message . $url;
    $sent = wp_mail($user_email, 'Resetar senha', $body);
...
function api_password_reset($request)
{
    ...
    if (is_wp_error($check_key)) {
        $response = new WP_Error('error', 'Token Inválido', ['status' => 401]);
        return rest_ensure_response($response);
    }
